...
pub struct MaestroDTO {
    pub id: String, pub nombre: String, pub color: String, pub activo: bool,
}

pub async fn obtener_categorias_inversiones(State(pool): State<PgPool>) -> impl IntoResponse {
    let rows: Vec<MaestroDTO> = sqlx::query_as("SELECT id::text, nombre, color, activo FROM categorias WHERE tipo_operacion_id = 'AHORRO' ORDER BY nombre")
        .fetch_all(&pool).await.unwrap_or_default();
    Json(rows)
}

pub async fn obtener_cuentas_inversiones(State(pool): State<PgPool>) -> impl IntoResponse {
    let rows: Vec<MaestroDTO> = sqlx::query_as("SELECT c.id::text, c.nombre, c.color, c.activo FROM cuentas c JOIN cuentas_tipos_operacion ct ON c.id = ct.cuenta_id WHERE ct.tipo_operacion_id = 'AHORRO' ORDER BY c.nombre")
        .fetch_all(&pool).await.unwrap_or_default();
    Json(rows)
}

pub async fn listar_inversiones(State(pool): State<PgPool>) -> impl IntoResponse {
    let rows: Vec<InversionDTO> = sqlx::query_as(
        "SELECT o.id::text, o.fecha::text, o.cantidad::float, o.categoria_id::text, c.nombre as categoria, c.color, o.cuenta_id::text, cu.nombre as cuenta, o.notas, o.pendiente 
         ...
         WHERE o.tipo_operacion_id = 'AHORRO' ORDER BY o.fecha DESC"
    ).fetch_all(&pool).await.unwrap_or_default();
    Json(rows)
}

pub async fn crear_inversion(State(pool): State<PgPool>, Json(payload): Json<UpsertInversionDTO>) -> impl IntoResponse {
    sqlx::query("INSERT INTO operaciones (tipo_operacion_id, fecha, cantidad, categoria_id, cuenta_id, notas, pendiente) VALUES ('AHORRO', $1::date, $2::float, $3::uuid, $4::uuid, $5, $6::boolean)")
...
// ACCIÓN COMPLETAR / ACTIVAR (Válida para Gastos, Ingresos o Inversiones)
pub async fn completar_operacion(State(pool): State<PgPool>, Path(id): Path<String>) -> impl IntoResponse {
    sqlx::query("UPDATE operaciones SET pendiente = false WHERE id = $1::uuid").bind(&id).execute(&pool).await.ok();
    StatusCode::OK
}